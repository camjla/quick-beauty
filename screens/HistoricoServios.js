...
const HistoricoServios = () => {
  ...
  const [filtrarContainerVisible, setFiltrarContainerVisible] = useState(false);
  ...
  const openFiltrarContainer = useCallback(() => {
    setFiltrarContainerVisible(true);
  }, []);

  const closeFiltrarContainer = useCallback(() => {
    setFiltrarContainerVisible(false);
  }, []);

  return (
    <>
      <View style={styles.historicoServios}>
        <ServiceCard />
        <FormContainer
          imageDimensions={require("../assets/ellipse-201.png")}
          propTop={193}
          propLeft={11}
          onFramePressablePress={openFrame}
        />
        <Pressable
          style={[styles.filtrar, styles.navbarShadowBox]}
          onPress={openFiltrarContainer}
        >
          <View style={[styles.anterioresParent, styles.navbarFlexBox]}>
            <Text style={styles.anteriores}>Anteriores</Text>
            <Image
              style={styles.materialSymbolskeyboardArroIcon}
              contentFit="cover"
              source={require("../assets/materialsymbolskeyboardarrowup.png")}
            />
          </View>
        </Pressable>
        <Text style={[styles.histricoDeServios, styles.serviosTypo]}>
        ...
        <View style={[styles.navbar, styles.navbarFlexBox]}>
          ...
          <Text style={[styles.servios, styles.serviosTypo]}>Serviços</Text>
        </View>
      </View>
      ...
      <Modal animationType="fade" transparent visible={filtrarContainerVisible}>
        <View style={styles.filtrarContainerOverlay}>
          <Pressable
            style={styles.filtrarContainerBg}
            onPress={closeFiltrarContainer}
          />
          <ModalEscolhaUmTipo onClose={closeFiltrarContainer} />
        </View>
      </Modal>
    </>
  );
...
export default HistoricoServios;
